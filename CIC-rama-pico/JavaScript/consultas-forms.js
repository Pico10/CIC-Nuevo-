  ...
  try {
    ...
    snap.forEach((d) => {
      ...
      const obj = {
        ...
        telefono: p.telefono || p.celular || p.phone || "", // ✅ IMPORTANTE para citación
      };
      ...
      opt.dataset.telefono = obj.telefono || "";
...
// ✅ NUEVO: citación
function toggleCitacionUI() {
  const chk = $("requiereAsistencia");
  const fecha = $("citacionFecha");
  const hora = $("citacionHora");
  const box = $("citacionBox");

  if (!chk || !fecha || !hora) return;
...
function validateForm() {
  ...
  if (descripcion.length < 20) {
    showStatus("❌ La descripción debe tener al menos 20 caracteres", "error");
    $("descripcion")?.focus();
    return false;
  }

  // ✅ NUEVO: validar citación si requiere asistencia
  const requiereAsistencia = $("requiereAsistencia")?.checked || false;
  const citFecha = ($("citacionFecha")?.value || "").trim();
  const citHora = ($("citacionHora")?.value || "").trim();

  if (requiereAsistencia) {
    if (!citFecha) {
      showStatus("❌ Debe indicar el día de la citación", "error");
      $("citacionFecha")?.focus();
      return false;
    }
    if (!citHora) {
      showStatus("❌ Debe indicar el horario de la citación", "error");
      $("citacionHora")?.focus();
      return false;
    }
  }
...
function buildConsultaPayload() {
  ...
  const personaTelefono = opt.dataset.telefono || ""; // ✅ NUEVO (si está)
  ...
  if (requiereAsistencia) {
    payload.citacion = {
      requiere: true,
      fecha: citFecha || null, // YYYY-MM-DD
      hora: citHora || null, // HH:MM
      asignadaAt: ahora, // ✅ para el WhatsApp
      asignadaPor: usuario,
    };
  } else {
    payload.citacion = {
...
async function getNextConsultaNumber() {
  const counterRef = doc(db, "systems", "consultas");
...
async function saveConsulta(payload) {
  ...
  if (payload.profesionalAsignado) {
    try {
      ...
      await updateDoc(profRef, { casosActivos: increment(1) });
    } catch (err) {
      console.warn("No se pudo actualizar contador de casos del profesional:", err);
    }
  }
...
/* ============================================================
   WHATSAPP: MODAL DE CONFIRMACIÓN + ENVÍO (AL PROFESIONAL)
   ============================================================ */
...
*Prioridad:* ${String(consulta.prioridad || "media").toUpperCase()}
*Tipo:* ${
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (personSelect) personSelect.addEventListener("change", showPersonInfo);

  const tipoSelect = $("tipo");
  if (tipoSelect) tipoSelect.addEventListener("change", toggleDerivadoPor);

  const motivoSelect = $("motivo");
  if (motivoSelect) motivoSelect.addEventListener("change", toggleOtroMotivo);

  // ✅ NUEVO: citación UI
  const chkAsistencia = $("requiereAsistencia");
  if (chkAsistencia) chkAsistencia.addEventListener("change", toggleCitacionUI);
  toggleCitacionUI();

  const form = $("consultaForm");
  if (form) form.addEventListener("submit", onSubmitConsulta);
});
